from flask import Flask, render_template, request, session, send_from_directory, jsonify, redirect, url_for, send_file
...
# Force the app to always run relative to this file's own folder.
# Without this, "AcciTrack.db" and "uploads" (used as relative paths
# throughout this file) resolve differently depending on where the
# app happens to be launched from, which is how multiple divergent
# copies of AcciTrack.db ended up scattered across the project.
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
os.chdir(BASE_DIR)

separator_string = "[sprtr_str]"
# Report evidence uploads must be video files only.
ALLOWED_EXTENSIONS = {"mp4", "mov", "avi", "wmv", "mkv", "webm", "flv", "m4v", "3gp", "3g2", "mpeg", "mpg", "ogv", "ogg", "ts", "m2ts"}
UPLOAD_FOLDER = os.path.join(os.getcwd(), "uploads")

app = Flask(__name__)

# Holds the most recent instant crash alert pushed by the Pi (see
# /api/pi-crash-alert below). This fires the moment the Pi's crash
# heuristic trips, BEFORE the full video clip is recorded/uploaded, so the
# admin dashboard can flip a "crash detected" indicator right away instead
# of waiting for the full report to land via /api/pi-crash-report. It's
# in-memory only (not persisted to the DB) since it's meant to be a
# short-lived, live signal, not a permanent record - the full report with
# the video and the same lat/lng still gets filed separately, moments
# later, once the clip is ready.
latest_pi_alert = {
    "active": False,
    "location": None,
    "lat": None,
...
def is_admin_badge(badge):
    """
    Server-side authority check: is this badge number actually an admin?
    Never trust the client for this — always re-check against the DB.
    """
    if not badge:
        return False
    try:
...
def resolve_profile_picture_path(badge):
    """
    Return the URL for a badge's current avatar, or an empty string if
    the officer hasn't uploaded one yet. Used both by the
    /get-profile-picture endpoint (for JS-driven updates) and by page
    routes (so the <img> tag is already correct on first render, with no
    fetch-then-swap flash). Callers should treat an empty return as
    "no photo uploaded" and show a blank placeholder instead of any
    default image.
    """
...
# --- PIN / PASSWORD HASHING HELPERS ---
# Werkzeug's hashes always look like "<method>:<salt-and-params>$<hash>",
# e.g. "scrypt:32768:8:1$abc123$def456...". Plaintext PINs never contain
# that "$" separator, so we can tell the two apart to support a smooth
# migration for any accounts that were seeded/created before hashing existed.

def hash_pin(raw_pin):
    """Hash a plaintext PIN/password for storage. Never store raw_pin directly."""
    return generate_password_hash(str(raw_pin))


def _looks_hashed(stored_value):
    stored_value = str(stored_value)
    return "$" in stored_value and (
        stored_value.startswith("scrypt:")
        or stored_value.startswith("pbkdf2:")
    )


def verify_pin(stored_value, provided_pin):
    """
    Verify a provided PIN against what's stored.
    Returns (matches, needs_rehash) so callers can transparently upgrade
    any legacy plaintext value to a hash the moment it's confirmed correct.
    """
    if _looks_hashed(stored_value):
        return check_password_hash(str(stored_value), str(provided_pin)), False

    # Legacy plaintext value (e.g. from before hashing was added, or a
    # freshly-seeded demo account). Fall back to a direct comparison,
    # and signal that the caller should upgrade it to a hash on success.
    matches = str(stored_value).strip() == str(provided_pin).strip()
...
def increment_failed_attempt(badge_number, officer_email=None):
    """
    Tracks and increments failed login attempts in the database (shared across
    processes/workers and persists across restarts). Locks account for 1 minute
    on 3 failed attempts and notifies their primary email address.
    """
    ...
    try:
        cursor.execute(
            "SELECT attempts FROM AcciTrack_LoginLockouts WHERE badge_number = ?",
            (badge_number,)
        )
        row = cursor.fetchone()
        ...
        if attempts >= 3:
            lockout_until = (now + datetime.timedelta(minutes=1)).isoformat()
            cursor.execute("""
                INSERT INTO AcciTrack_LoginLockouts (badge_number, attempts, lockout_until)
                VALUES (?, 0, ?)
                ON CONFLICT(badge_number) DO UPDATE SET attempts = 0, lockout_until = ?
            """, (badge_number, lockout_until, lockout_until))
            conn.commit()

            # Send an email warning to their registered address
            if officer_email:
                subject = "AcciTrack Security Alert: Suspicious Login Activity Detected"
                body = (
                    f"Hello Officer,\n\n"
                    f"We detected 3 consecutive failed login attempts on your AcciTrack account (Badge #{badge_number}).\n"
                    f"As a security precaution, further login attempts for this badge number have been suspended for 1 minute.\n\n"
                    f"If you did not initiate these login attempts, someone else might be trying to access your account. "
                    f"Please update your password PIN immediately or notify your system administrator.\n\n"
                    f"Best regards,\n"
                    f"AcciTrack System Security"
                )
                send_smtp_email(officer_email, subject, body)
        else:
            cursor.execute("""
                INSERT INTO AcciTrack_LoginLockouts (badge_number, attempts, lockout_until)
                VALUES (?, ?, NULL)
                ON CONFLICT(badge_number) DO UPDATE SET attempts = ?
            """, (badge_number, attempts, attempts))
            conn.commit()
    finally:
        conn.close()


def check_lockout(badge_number):
    """
    Returns remaining lockout seconds (int) if the badge is currently locked out,
    otherwise returns None. Reads from the database so it's consistent across
    worker processes and survives server restarts.
    """
    conn = sqlite3.connect("AcciTrack.db")
    cursor = conn.cursor()
    try:
        cursor.execute(
            "SELECT lockout_until FROM AcciTrack_LoginLockouts WHERE badge_number = ?",
            (badge_number,)
        )
        row = cursor.fetchone()
        if not row or not row[0]:
            return None
        lockout_until = datetime.datetime.fromisoformat(row[0])
        now = datetime.datetime.now()
...
def clear_failed_attempts(badge_number):
    """Resets the failed-attempt counter and lockout for a badge after a successful login."""
    conn = sqlite3.connect("AcciTrack.db")
    cursor = conn.cursor()
    try:
        cursor.execute("DELETE FROM AcciTrack_LoginLockouts WHERE badge_number = ?", (badge_number,))
        conn.commit()
    finally:
        conn.close()
...
def client():
    ...
    if is_admin_badge(session.get("badge_number")):
        # Admins get sent to their own dashboard instead of the officer/client
        # view, symmetric with how /admin sends non-admins to /client.
    ...
    return render_template(
        "client.html",
        officer_profile_picture=resolve_profile_picture_path(officer_data[9]),
...
def camera():
    ...
    back_url = url_for('admin') if is_admin else url_for('client')
    return render_template("camera.html", back_url=back_url)
...
def admin():
    if session.get("logged_in", "no") == "no":
        return redirect(url_for('login'))
    if not is_admin_badge(session.get("badge_number")):
        # Non-admins (officers/clients) get sent to their own dashboard
        # instead of being able to view the admin interface at all.
...
def log_in():
    try:
        ...
        # First, check whether an account matching this badge number/username
        # exists at all, BEFORE running any lockout / failed-attempt logic.
        # This avoids creating lockout records for badge numbers that don't
        ...
        for officer in db.get_table_values("AcciTrack", "AcciTrack_OfficerList"):
            if officer[9] == badge_number or officer[30] == username:
                ...
                # Lockout check now happens only after we've confirmed the
                # account exists, and before we attempt authentication.
                remaining_seconds = check_lockout(badge_number)
                if remaining_seconds is not None:
                    return f"lockout{separator_string}{remaining_seconds}"

                if officer[9] != badge_number:
                    increment_failed_attempt(badge_number, officer[11])
                ...
                pin_ok, needs_rehash = verify_pin(officer[29], officer_pin)
                if pin_ok:
                    if needs_rehash:
                        # Transparently upgrade a legacy plaintext PIN to a hash
                        # now that we've confirmed it's correct.
                        try:
                            upgrade_conn = sqlite3.connect("AcciTrack.db")
                            upgrade_cursor = upgrade_conn.cursor()
                            upgrade_cursor.execute(
                                "UPDATE AcciTrack_OfficerList SET officer_pin = ? WHERE officer_badge_number = ?",
                                (hash_pin(officer_pin), officer[9]))
                            upgrade_conn.commit()
                            upgrade_conn.close()
                        except Exception as upgrade_err:
                            print("Error upgrading legacy PIN to hash:", upgrade_err)

                    # Reset failed attempts upon successful login
                    clear_failed_attempts(officer[9])
                    ...
                    increment_failed_attempt(officer[9], officer[11])
                    raise Exception("Incorrect password")

        if not officer_found:
            # No account matches this badge number/username, so there's
            # nothing to lock out — skip the 3-attempt tracking entirely.
...
def add_report():
    try:
        ...
        location_lng = request.args.get("lng", "")
        ...
        if 'file' in request.files:
            ...
            if file.filename == '':
                return "Fail"
            # Only video files are accepted for report evidence. Some browsers
            # send an unreliable/missing content_type for formats like
            # .mkv/.avi, so fall back to extension checking (mirrors the
            # client-side isVideoFile() logic).
            mime_ok = bool(file.content_type) and file.content_type.startswith("video/")
            ext_ok = allowed_file(file.filename)
            if file and (mime_ok or ext_ok):
        ...
        values = [
            ...
            {"report_location_lat": location_lat},
            {"report_location_lng": location_lng}
        ]
...
def admin_get_reports():
    # Despite the name, this feeds shared department-wide report data to
    # BOTH the admin and officer/client dashboards (Active Cases, incident
    # counts, etc.), so it only needs to require login — not admin rights.
    if session.get("logged_in", "no") == "no":
        return "Unauthorized"
...
def change_pin():
    try:
        ...
        current_ok, _ = verify_pin(row[0], current_pin)
        if not current_ok:
            conn.close()
...
@app.route('/api/pi-crash-report', methods=["POST"])
def pi_crash_report():
    """
    Receives an automatically-detected crash clip pushed by the Raspberry Pi
    detection script (detect_and_stream.py) and files it as a report, same
    as a manually-submitted one, so it shows up for admins to review.
    Auth: X-API-KEY header must match the PI_API_KEY env var.
    """
    expected_key = os.environ.get("PI_API_KEY")
    ...
    try:
        ...
        values = [
            ...
            {"report_reviewing_reason": "none"},
            {"report_is_read": "no"},
            {"report_officer_badge_number": "PI-SYSTEM"},
            {"report_real_submission_datetime": submitting_datetime},
            {"report_location_lat": location_lat},
            {"report_location_lng": location_lng}
        ]
        db.insert_to_table("AcciTrack", "AcciTrack_ReportList", values)

        log_security_event("PI-SYSTEM", "Report Submitted",
                            f"AcciTrack-Pi automatically submitted a crash report. Case Ref: {case_num}, Location: {location}")

        return jsonify({"status": "Success", "caseNum": case_num}), 200
    except Exception as e:
        print("Pi crash report error:", str(e))
        return jsonify({"error": "Server error"}), 500


@app.route('/api/pi-crash-alert', methods=["POST"])
def pi_crash_alert():
    """
    Instant "a crash just happened" notification from the Pi, sent the
    moment its crash heuristic trips - well before the full video clip
    has finished recording/uploading via /api/pi-crash-report. Lets the
    admin dashboard flip a live indicator right away instead of waiting
    the several extra seconds the full clip pipeline takes.

    Auth: same as /api/pi-crash-report - X-API-KEY header must match the
    PI_API_KEY env var.

    Body (JSON): { "location": str, "lat": float|str, "lng": float|str,
                   "timestamp": iso8601 str }
    "lat"/"lng" are optional - if the Pi couldn't get a GPS fix and has no
    fallback coordinates configured, they'll simply be omitted/None and
    only the text location label is shown.
    """
    expected_key = os.environ.get("PI_API_KEY")
    provided_key = request.headers.get("X-API-KEY")
    if not expected_key or not provided_key or provided_key != expected_key:
    ...
    try:
        ...
        location = data.get("location", "Unknown (Pi camera)")
        lat = data.get("lat")
        lng = data.get("lng")
        timestamp = data.get("timestamp") or datetime.datetime.now().isoformat()

        latest_pi_alert.update({
            "active": True,
            "location": location,
            "lat": lat,
            "lng": lng,
        ...
        log_security_event("PI-SYSTEM", "Crash Alert",
                            f"AcciTrack-Pi flagged a possible crash at {location} "
                            f"(lat={lat}, lng={lng}). Full report to follow shortly.")

        return jsonify({"status": "Success"}), 200
    except Exception as e:
        print("Pi crash alert error:", str(e))
        return jsonify({"error": "Server error"}), 500


@app.route('/api/pi-crash-alert/latest', methods=["GET"])
def pi_crash_alert_latest():
    """
    Lets the admin dashboard poll for the most recent instant crash alert
    (see /api/pi-crash-alert above). Requires an active admin session,
    same as the rest of the dashboard's data endpoints.
    """
    if session.get("logged_in", "no") == "no":
        return jsonify({"error": "Unauthorized"}), 401
    return jsonify(latest_pi_alert)
...
def get_profile_picture():
    try:
        if session.get("logged_in", "no") == "no":
            return "Unauthorized"
        badge = session.get("badge_number")
        return resolve_profile_picture_path(badge)
    except Exception as e:
...
def seed_security_tables():
    try:
        ...
        """)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS AcciTrack_LoginLockouts (
                badge_number TEXT PRIMARY KEY,
    ...
    try:
        ...
        if "1" not in existing_badges and "cafe1030" not in existing_usernames:
            officer_data = [
                ...
                {"officer_employment_history": ""},
                {"officer_document_list": ""},
                {"officer_pin": hash_pin("Manhattan@2026")},  # Seeded with complex password (hashed at rest)
        ...
        if "2" not in existing_badges and "oogabooga" not in existing_usernames:
            admin_data = [
                ...
                {"officer_document_list": ""},
                {"officer_pin": hash_pin("McQueen#2026")},  # Seeded with complex password (hashed at rest)
...
@app.route('/export-all-reports-excel', methods=["GET", "POST"])
def export_all_reports_excel():
    if session.get("logged_in", "no") == "no":
        return "Unauthorized", 401

    # Verify current user possesses administrator privileges
    badge = session.get("badge_number")
    conn = sqlite3.connect("AcciTrack.db")
    cursor = conn.cursor()
    cursor.execute("SELECT officer_is_admin FROM AcciTrack_OfficerList WHERE officer_badge_number = ?", (badge,))
    ...
    if not row or row[0] != "yes":
        conn.close()
        return "Unauthorized: Administrator privileges required", 403

    # Query all reports joined with the submitting officer's full credentials
    cursor.execute("""
        SELECT 
            r.report_caseNum,
            r.report_type,
            r.report_location,
            r.report_status,
            r.report_real_submission_datetime,
            r.report_submitting_datetime,
            r.report_reviewing_officer,
            r.report_reviewing_datetime,
            r.report_reviewing_reason,
            o.officer_first_name,
            o.officer_middle_name,
            o.officer_last_name,
            o.officer_preferred_name,
            o.officer_badge_number,
            o.officer_employee_id,
            o.officer_username,
            o.officer_employment_job_title,
            o.officer_employment_department,
            o.officer_employment_type,
            o.officer_employment_start_date,
            o.officer_employment_work_location,
            o.officer_employment_reporting_officer,
            o.officer_birthday,
            o.officer_gender,
            o.officer_nationality,
            o.officer_blood_type,
            o.officer_social_security_number,
            o.officer_primary_email,
            o.officer_work_phone,
            o.officer_mobile_phone,
            o.officer_primary_contact_name,
            o.officer_primary_contact_phone_number,
            o.officer_primary_contact_relationship
        FROM AcciTrack_ReportList r
        LEFT JOIN AcciTrack_OfficerList o ON r.report_officer_badge_number = o.officer_badge_number
    """)
    raw_reports = cursor.fetchall()
    conn.close()

    # Sort reports chronologically from newest to oldest
    def sort_key(item):
        dt = parse_datetime(item[4]) or parse_datetime(item[5])
        return dt or datetime.datetime.min

    sorted_reports = sorted(raw_reports, key=sort_key, reverse=True)

    headers = [
        "Case Number", "Incident Type", "Incident Location", "Status",
        "Submission Date/Time", "Reviewing Admin", "Review Date/Time", "Review Notes/Reason",
        "Officer First Name", "Officer Middle Name", "Officer Last Name", "Preferred Name",
        "Badge Number", "Employee ID", "Username", "Rank/Title", "Department",
        "Employment Type", "Hire Date", "Station/Work Location", "Reporting Manager",
        "Date of Birth", "Gender", "Nationality", "Blood Type", "SSN",
        "Primary Email", "Work Phone", "Mobile Phone",
        "Emergency Contact Name", "Emergency Contact Phone", "Emergency Contact Relationship"
    ]

    import io

    # Attempt to output format as openpyxl Excel spreadsheet first
    try:
        import openpyxl
        ...
        header_fill = PatternFill(start_color="0B2C66", end_color="0B2C66", fill_type="solid")
        ...
        filename = f"AcciTrack_Reports_Export_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        return send_file(
            out,
        ...
        # Fall back gracefully to standard CSV structure with UTF-8 BOM
        import csv
        ...
        output.seek(0)
        filename = f"AcciTrack_Reports_Export_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        return send_file(
...
def migrate_report_location_columns():
    """
    Adds report_location_lat / report_location_lng columns to an
    already-existing AcciTrack_ReportList table. EasySQL.create_table only
    runs CREATE TABLE IF NOT EXISTS, so on a database that was created before
    these columns existed, they need to be added on with ALTER TABLE instead.
    Safe to call every time the app starts: it checks for the columns first.
    """
    ...
    try:
        conn = sqlite3.connect("AcciTrack.db")
        cursor = conn.cursor()
        cursor.execute("PRAGMA table_info(AcciTrack_ReportList)")
        existing_columns = {row[1] for row in cursor.fetchall()}
        for column_name in ("report_location_lat", "report_location_lng"):
            if column_name not in existing_columns:
                cursor.execute(f"ALTER TABLE AcciTrack_ReportList ADD COLUMN {column_name} text")
        conn.commit()
        conn.close()
    except Exception as e:
        print(f"[AcciTrack] Report location column migration failed: {e}")
...
if "__main__" == __name__:
    ...
    migrate_report_location_columns()
    seed_custom_users()
