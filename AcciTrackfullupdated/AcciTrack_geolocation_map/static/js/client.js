...
const CLIENT_TAB_STORAGE_KEY = "accitrack_client_last_tab";
const CLIENT_TAB_COUNT = 5;

function getLastActiveTab() {
    try {
        const stored = parseInt(localStorage.getItem(CLIENT_TAB_STORAGE_KEY), 10);
        if (!isNaN(stored) && stored >= 0 && stored < CLIENT_TAB_COUNT) {
            return stored;
        }
    } catch (e) {
...
function tabClick(index){
    ...
    try {
        localStorage.setItem(CLIENT_TAB_STORAGE_KEY, String(index));
    } catch (e) {
...
function openReportModal() {
    ...
    accidentTypeSelect.disabled = false;
    ...
    dropArea.classList.remove("dragover", "file-error");
    ...
    // Wire up real drag & drop (previously only visual, not functional)
...
// Restore the "use my location" button to its default (non-loading) state
// Clears the hidden lat/lng once the officer starts typing over an
// auto-filled location, so we never submit coordinates that no longer match
// the typed address.
...
// Shows a small embedded map right on the report form, centered on the
// given coordinates, so the officer can visually confirm the pin is in the
// right spot before submitting.
function showLocationMapPreview(lat, lng) {
    const preview = document.getElementById("locationMapPreview");
    const frame = document.getElementById("locationMapPreviewFrame");
    if (!preview || !frame) return;
    const delta = 0.006;
    const bbox = `${lng - delta},${lat - delta},${lng + delta},${lat + delta}`;
    frame.src = `https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&marker=${lat},${lng}&layer=mapnik`;
...
// Uses the browser's HTML5 Geolocation API to grab the reporting officer's
// current coordinates, reverse-geocodes them into a human-readable address,
// and drops the result straight into the Accident Location field. Since that
// field is what gets submitted to the server via /add-report, this is how
// the client's location makes it over to the admin side.
function useCurrentLocation() {
    ...
    if (!("geolocation" in navigator)) {
        alert("Geolocation isn't supported by this browser. Please type the location manually.");
        return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
        async function (position) {
            ...
            // Store the raw coordinates so they can be sent to the server
            // and shown as a real pin on a map on the admin side, not just
            // as descriptive text.
            latInput.value = lat;
            lngInput.value = lng;
            showLocationMapPreview(lat, lng);
            ...
            try {
                ...
                // Reverse geocoding failed (offline, rate-limited, etc.) —
                // fall back to raw coordinates so the report can still be
                // filed with an accurate location.
        ...
        function (error) {
            ...
            if (error.code === error.PERMISSION_DENIED) {
                message = "Location access was denied. Please allow location permissions and try again, or type the location manually.";
            } else if (error.code === error.POSITION_UNAVAILABLE) {
                message = "Your location is currently unavailable. Please try again or type the location manually.";
            } else if (error.code === error.TIMEOUT) {
                message = "Getting your location took too long. Please try again or type the location manually.";
            }
            alert(message);
        },
...
// Accepted video extensions, used as a fallback when a file's MIME type is
// missing or unreliable (common with .mkv/.avi dragged in from some OSes).
const ALLOWED_VIDEO_EXTENSIONS = [
    "mp4", "mov", "avi", "wmv", "mkv", "webm", "flv", "m4v", "3gp", "3g2",
    "mpeg", "mpg", "ogv", "ogg", "ts", "m2ts"
];
...
function handleSelectedVideoFile(file) {
    ...
    if (!isVideoFile(file)) {
        ...
        document.getElementById("videoPreview").innerHTML = `
            <p style="margin:8px 0 0; color:#ef4444; font-size:14px;">
                ...
                <strong>${escapeHtml(file.name)}</strong> is not a supported video file.
                Please upload a video (MP4, MOV, AVI, WMV, MKV, WEBM, etc.).</p>`;

        dropArea.classList.add("file-error");
        setTimeout(() => dropArea.classList.remove("file-error"), 1500);
        return;
    }
    ...
    document.getElementById("videoPreview").innerHTML = `
        ...
        <p style="margin:8px 0 0; color:#475569; font-size:14px;">
            <strong>${escapeHtml(file.name)}</strong> (${(file.size/1024/1024).toFixed(2)} MB)</p>`;
}
...
function openReportModalWithTemplate(type) {
    ...
    if (selectElement) {
        ...
        // Lock the accident type to the chosen template — officers filing
        // via a Quick Template shouldn't be able to switch it to a
        // different incident type. Only a plain "New Report" (no template)
        // leaves this editable.
...
function addReport(){
    ...
    try {
        ...
        req.open("POST", `/add-report?caseNum=${caseNum}&officer=${encodeURIComponent(document.getElementById("officerName").value.trim())}&datetime=${encodeURIComponent(dateVal)}&location=${encodeURIComponent(document.getElementById("location").value.trim())}&type=${encodeURIComponent(document.getElementById("accidentType").value.trim())}&status=pending&video=none&realdatetime=${encodeURIComponent(dateVal)}&lat=${encodeURIComponent(lat)}&lng=${encodeURIComponent(lng)}`, true);
...
function injectUserManualButton() {
    ...
    if (profileActions && !document.getElementById("btnUserManual")) {
        const btn = document.createElement("button");
        btn.type = "button";
        btn.id = "btnUserManual";
        btn.className = "btn btn-outline";
        btn.style.cssText = "display: inline-flex; align-items: center; gap: 8px; font-weight: 600; cursor: pointer;";
...
function renderManualPage(page) {
    ...
    if (page === 1) {
        titleText = "I. Introduction & System Navigation";
        bodyContent = `
            <p style="color: #64748b; font-size: 14px; margin-bottom: 20px;">Welcome, Officer. This guide walks you through the workflow of on-duty operations. Here is your standard operational navigation flow:</p>

            <!-- Corrected Navigation Sidebar Flowchart -->
            <div style="display: flex; align-items: center; justify-content: space-between; margin: 16px 0 24px; background: #f8fafc; padding: 16px 12px; border-radius: 14px; border: 1.5px solid #cbd5e1; overflow-x: auto; gap: 8px; box-sizing: border-box; width: 100%;">
                ...
                <div style="display: flex; flex-direction: column; align-items: center; text-align: center; flex: 1; min-width: 75px; flex-shrink: 0; padding: 4px 0;">
                    <div style="width: 42px; height: 42px; background: #0b2c66; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 6px; font-size: 16px; box-shadow: 0 4px 10px rgba(11,44,102,0.15); flex-shrink: 0;">
                        <i class="fa-solid fa-gauge-high"></i>
                    </div>
                    <span style="font-size: 11px; font-weight: 700; color: #0f172a; white-space: nowrap;">1. Dashboard</span>
                </div>
                ...
                <div style="display: flex; flex-direction: column; align-items: center; text-align: center; flex: 1; min-width: 75px; flex-shrink: 0; padding: 4px 0;">
                    ...
                    <span style="font-size: 11px; font-weight: 700; color: #0f172a; white-space: nowrap;">2. Incidents</span>
                </div>
                <i class="fa-solid fa-chevron-right" style="color: #94a3b8; font-size: 11px; flex-shrink: 0; align-self: center; margin-bottom: 14px;"></i>
                ...
                <div style="display: flex; flex-direction: column; align-items: center; text-align: center; flex: 1; min-width: 75px; flex-shrink: 0; padding: 4px 0;">
                    ...
                    <span style="font-size: 11px; font-weight: 700; color: #0f172a; white-space: nowrap;">3. Reports</span>
                </div>
                <i class="fa-solid fa-chevron-right" style="color: #94a3b8; font-size: 11px; flex-shrink: 0; align-self: center; margin-bottom: 14px;"></i>
                ...
                <div style="display: flex; flex-direction: column; align-items: center; text-align: center; flex: 1; min-width: 75px; flex-shrink: 0; padding: 4px 0;">
                    ...
                    <span style="font-size: 11px; font-weight: 700; color: #0f172a; white-space: nowrap;">4. Profile</span>
                </div>
                <i class="fa-solid fa-chevron-right" style="color: #94a3b8; font-size: 11px; flex-shrink: 0; align-self: center; margin-bottom: 14px;"></i>
                ...
                <div style="display: flex; flex-direction: column; align-items: center; text-align: center; flex: 1; min-width: 75px; flex-shrink: 0; padding: 4px 0;">
                    ...
                    <span style="font-size: 11px; font-weight: 700; color: #0f172a; white-space: nowrap;">5. Notifications</span>
                </div>
            </div>
            ...
            <h4 style="color: #0b2c66; font-size: 16px; font-weight: 800; margin-bottom: 14px; text-align: left; display: flex; align-items: center; gap: 8px;">
                <i class="fa-solid fa-bars-staggered" style="color: #0b2c66;"></i> Navigation Sidebars
            </h4>
            <div style="display: flex; flex-direction: column; gap: 12px; text-align: left;">
                <div style="display: flex; align-items: flex-start; gap: 12px;">
                    ...
                    <div><strong>Dashboard</strong>: Telemetry metrics tracking resolved counts, response averages, and checklist tasks.</div>
                </div>
                <div style="display: flex; align-items: flex-start; gap: 12px;">
                    ...
                    <div><strong>Incidents</strong>: Interactive breakdown cards, keyword queries, and layouts to study global footage.</div>
                </div>
                <div style="display: flex; align-items: flex-start; gap: 12px;">
                    ...
                    <div><strong>Reports</strong>: Quick templates and report filing containing body camera video evidence.</div>
                </div>
                <div style="display: flex; align-items: flex-start; gap: 12px;">
                    ...
                    <div><strong>Profile</strong>: Personal identity sheets, hire records, history timeline updates, and personal document safe.</div>
                </div>
                <div style="display: flex; align-items: flex-start; gap: 12px;">
                    <i class="fa-solid fa-bell" style="color: #0b2c66; font-size: 16px; margin-top: 3px; width: 20px; text-align: center; flex-shrink: 0;"></i>
                    <div><strong>Notifications</strong>: Status tracking for filed reports along with administrative feedback.</div>
                </div>
            </div>
        `;
    } else if (page === 2) {
        titleText = "II. Dashboard & Checklist Management";
        bodyContent = `
            <p style="color: #64748b; font-size: 14px; margin-bottom: 20px;">Configure daily checkpoints and study live on-duty performance metrics directly from the main view panel:</p>
            <div style="display: flex; flex-direction: column; gap: 14px; text-align: left;">
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1.5px solid #cbd5e1; box-sizing: border-box;">
                    ...
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Click "Add New Task", input checkpoint details, select a priority classification, and save. Click "X" to clear items.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1.5px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-chart-line" style="color: #0b2c66; font-size: 16px;"></i> Live Telemetry Overview
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Track key precinct performance across four live stat cards: total Recent Incidents (with daily comparison), Resolved Cases approved today, active Personnel on Duty, and computed Average Response Time.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1.5px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-video" style="color: #0b2c66; font-size: 16px;"></i> Recent Incidents & Active Cases
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Monitor incoming body camera reports submitted from the field. Click any listed Active Case or Recent Incident card to open its complete location map, timestamps, and officer filing details.</p>
                </div>
            </div>
        `;
    } else if (page === 3) {
        ...
        bodyContent = `
            <p style="color: #64748b; font-size: 14px; margin-bottom: 20px;">Explore, filter, and study recorded body camera footage compiled across your sector:</p>
            <div style="display: flex; flex-direction: column; gap: 14px; text-align: left;">
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1.5px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-table-cells-large" style="color: #0b2c66; font-size: 16px;"></i> Accident Category Filters
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Click any of the 6 category breakdown cards at the top (Minor Traffic, Reckless Driving, DUI/DWI, Hit & Run, Multi-Vehicle Pileup, or Homicide) to filter the video grid by accident type. Click the card again to reset the filter.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1.5px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-magnifying-glass" style="color: #0b2c66; font-size: 16px;"></i> Search & Sorting Controls
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Use the search bar to instantly find footage by officer name, street location, or case ID. Refine the video grid using time range filters (Today, 24 Hours, 7 Days, 30 Days) or sort entries chronologically.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1.5px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
        ...
        titleText = "IV. Filing Accident Incident Reports";
        bodyContent = `
            <p style="color: #64748b; font-size: 14px; margin-bottom: 20px;">Submit detailed on-scene cases securely using verified template procedures:</p>
            <div style="display: flex; flex-direction: column; gap: 14px; text-align: left;">
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1.5px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-file-circle-plus" style="color: #0b2c66; font-size: 16px;"></i> Selecting Templates
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Click any quick template card representing standard traffic violations to open a pre-filled layout form.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1.5px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-video" style="color: #0b2c66; font-size: 16px;"></i> Attaching Mandatory Footage
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Drag & drop or browse to attach the mandatory body camera video asset. Filing requires valid video evidence.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1.5px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-paper-plane" style="color: #0b2c66; font-size: 16px;"></i> Form Dispatching
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Enter location, verify auto-logged timestamps, and submit. The system generates a Case ID and dispatches it for review.</p>
                </div>
            </div>
        `;
    } else if (page === 5) {
        titleText = "V. Personal Profiles, Documents, & Password Safety";
        bodyContent = `
            <p style="color: #64748b; font-size: 14px; margin-bottom: 20px;">Access personal identity sheets and manage documents inside the profile settings panel:</p>
            <div style="display: flex; flex-direction: column; gap: 14px; text-align: left;">
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1.5px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-user-edit" style="color: #0b2c66; font-size: 16px;"></i> Profile Change Proposals
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Edit primary details, address sheets, emergencies, or upload photo avatars. Proposed edits are sent for Admin review.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1.5px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-folder-closed" style="color: #0b2c66; font-size: 16px;"></i> Secure Document Safe
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Upload, download, or delete personal credentials, licenses, or certification PDFs inside your personal vault.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1.5px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-key" style="color: #0b2c66; font-size: 16px;"></i> Password Modification
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Change password credentials securely. Passwords must be at least 8 characters with numbers, symbols, and uppercase letters.</p>
                </div>
            </div>
        `;
    }
    ...
    modal.innerHTML = `
        <div class="modal-content user-manual-content" style="background: #ffffff; border: 1.5px solid #cbd5e1; border-radius: 24px; width: 95%; max-width: 840px; max-height: 85vh; overflow: hidden; box-shadow: 0 25px 70px rgba(0,0,0,0.3); display: flex; flex-direction: column; font-family: 'Inter', sans-serif;">
            <!-- HEADER -->
            <div class="modal-header user-manual-header" style="background: #0b2c66; color: white; padding: 24px 32px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.08); flex-shrink: 0; position: relative;">
                <div style="display: flex; align-items: center; gap: 14px;">
                    <i class="fa-solid fa-book-open" style="font-size: 24px;"></i>
                    <h3 style="margin: 0; font-size: 20px; font-weight: 800; color: white;">${titleText}</h3>
                </div>
                <button type="button" onclick="closeUserManual()" style="all: unset; cursor: pointer; font-size: 28px; color: #94a3b8; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; transition: 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.1)'; this.style.color='white'" onmouseout="this.style.background='transparent'; this.style.color='#94a3b8'">&times;</button>
            </div>

            <!-- BODY -->
            <div class="modal-body user-manual-body" style="padding: 32px; overflow-y: auto; color: #1e293b; line-height: 1.6; font-size: 15px; flex: 1;">
                ${bodyContent}
            </div>

            <!-- FOOTER -->
            <div class="modal-footer user-manual-footer" style="padding: 20px 32px; background: #f8fafc; border-top: 1px solid #e2e8f0; flex-shrink: 0; display: flex; align-items: center; justify-content: space-between;">
                <button type="button" onclick="renderManualPage(${page - 1})" ${page === 1 ? 'disabled' : ''} style="all: unset; display: flex; align-items: center; gap: 8px; font-weight: 700; color: ${page === 1 ? '#cbd5e1' : '#0b2c66'}; cursor: ${page === 1 ? 'not-allowed' : 'pointer'}; transition: 0.2s;">
                    <i class="fa-solid fa-arrow-left"></i> Back
                </button>

                <div style="display: flex; gap: 8px;">
                    <span style="width: 8px; height: 8px; border-radius: 50%; background: ${page === 1 ? '#0b2c66' : '#cbd5e1'};"></span>
                    <span style="width: 8px; height: 8px; border-radius: 50%; background: ${page === 2 ? '#0b2c66' : '#cbd5e1'};"></span>
                    <span style="width: 8px; height: 8px; border-radius: 50%; background: ${page === 3 ? '#0b2c66' : '#cbd5e1'};"></span>
                    <span style="width: 8px; height: 8px; border-radius: 50%; background: ${page === 4 ? '#0b2c66' : '#cbd5e1'};"></span>
                    <span style="width: 8px; height: 8px; border-radius: 50%; background: ${page === 5 ? '#0b2c66' : '#cbd5e1'};"></span>
                </div>

                ${page < 5 ? `
                    <button type="button" onclick="renderManualPage(${page + 1})" style="all: unset; display: flex; align-items: center; gap: 8px; font-weight: 700; color: #0b2c66; cursor: pointer; transition: 0.2s;">
                        Next <i class="fa-solid fa-arrow-right"></i>
                    </button>
                ` : `
                    <button type="button" onclick="closeUserManual()" style="all: unset; background: #0b2c66; color: white; padding: 10px 24px; border-radius: 8px; font-weight: 700; cursor: pointer; transition: 0.2s;" onmouseover="this.style.background='#1e5799'" onmouseout="this.style.background='#0b2c66'">
                        Finish
                    </button>
                `}
            </div>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (dropdownTrigger && dropdownMenu) {
        const positionProfileDropdown = () => {
            ...
            const margin = 10;
            const menuWidth = Math.min(240, window.innerWidth - margin * 2);
            ...
            dropdownMenu.style.maxHeight = "calc(100vh - " + (top + margin) + "px)";
        ...
        window.addEventListener("resize", () => {
            if (dropdownMenu.style.display === "block") positionProfileDropdown();
        });
    ...
    // Wire up the incident-type breakdown cards (Minor Traffic, Reckless Driving, etc.)
    // so clicking one filters the incident grid below it, same as the admin dashboard.
