...
let activeIncidentTypeFilter = "all"; // Tracks selected breakdown card filter
...
function formatReadableDate(dateStr) {
    if (!dateStr || dateStr === "none" || dateStr === "null" || dateStr === "N/A") return "-";
...
function updateBreakdownCardSelectionVisuals(selectedType) {
    const cards = document.querySelectorAll(".accident-breakdown-row .breakdown-card");
    cards.forEach(card => {
        const type = card.getAttribute("data-type");
        if (type === selectedType) {
            card.style.borderColor = "#0b2c66";
            card.style.background = "#eff6ff";
            card.style.boxShadow = "0 0 0 4px rgba(11, 44, 102, 0.15)";
        } else {
...
const accidentIcons = {
    ...
    "Fire Alarms and Explosive-Related Incidents": { icon: "fa-fire-flame-curved", color: "#f59e0b" },
    "Damage to Property":                     { icon: "fa-house-chimney-crack", color: "#78716c" },
    "Theft / Robbery":                        { icon: "fa-mask",             color: "#9333ea" },
    "Physical Injuries":                      { icon: "fa-user-injured",     color: "#e11d48" }
};
...
const accidentTypes = [
    ...
    { type: "Fire Alarms and Explosive-Related Incidents", icon: "fa-solid fa-fire-flame-curved", color: "#f59e0b" },
    { type: "Damage to Property",             icon: "fa-solid fa-house-chimney-crack", color: "#78716c" },
    { type: "Theft / Robbery",                icon: "fa-solid fa-mask",                color: "#9333ea" },
    { type: "Physical Injuries",              icon: "fa-solid fa-user-injured",        color: "#e11d48" }
];
...
const ADMIN_TAB_STORAGE_KEY = "accitrack_admin_last_tab";
const ADMIN_TAB_COUNT = 6; // Dashboard, Incidents, Reports, Profile, Notifications, Users

function getLastActiveTab() {
    try {
        const stored = parseInt(localStorage.getItem(ADMIN_TAB_STORAGE_KEY), 10);
        if (!isNaN(stored) && stored >= 0 && stored < ADMIN_TAB_COUNT) {
            return stored;
        }
    } catch (e) {
        console.error("Error reading last active tab from localStorage:", e);
    }
    return 0;
}
...
function tabClick(index){
    ...
    try {
        localStorage.setItem(ADMIN_TAB_STORAGE_KEY, String(index));
    } catch (e) {
        console.error("Error saving last active tab to localStorage:", e);
    }
...
// Builds an embedded map (when we have real coordinates from the client's
// HTML5 Geolocation lookup) plus a "view larger map" link, falling back to a
// plain map-search link when only free-typed address text is available.
function buildLocationMapHTML(lat, lng, address, theme = "dark") {
    const hasCoords = lat && lng && lat !== "none" && lng !== "none" && !isNaN(parseFloat(lat)) && !isNaN(parseFloat(lng));
    const isLight = theme === "light";
    const footerBg = isLight ? "#f1f5f9" : "rgba(255,255,255,0.05)";
    ...
    if (hasCoords) {
        ...
        const embedSrc = `https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&marker=${latNum},${lngNum}&layer=mapnik`;
        const largeMapUrl = `https://www.google.com/maps?q=${latNum},${lngNum}`;
        return `
            <div style="margin-top:10px;border-radius:10px;overflow:hidden;border:1px solid ${borderColor};">
                <iframe src="${embedSrc}" style="width:100%;height:180px;border:0;display:block;" loading="lazy" title="Accident location map"></iframe>
                <a href="${largeMapUrl}" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:6px;justify-content:center;padding:8px;background:${footerBg};color:${linkColor};font-size:12.5px;font-weight:600;text-decoration:none;">
    ...
    // No coordinates on file (older report, or the officer typed the
    // location manually) — link out to a map search using the address text.
    const searchUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address || "")}`;
    return `
        <a href="${searchUrl}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:6px;margin-top:8px;color:${linkColor};font-size:12.5px;font-weight:600;text-decoration:none;">
            <i class="fa-solid fa-up-right-from-square"></i> Search this address on map
        </a>
    `;
}

// Compact variant for tight/repeating layouts (e.g. gallery cards) where an
// embedded iframe per card would be too heavy — just a map link.
function buildLocationMapLinkOnly(lat, lng, address) {
    const hasCoords = lat && lng && lat !== "none" && lng !== "none" && !isNaN(parseFloat(lat)) && !isNaN(parseFloat(lng));
    const url = hasCoords
        ? `https://www.google.com/maps?q=${parseFloat(lat)},${parseFloat(lng)}`
        : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address || "")}`;
    return `<a href="${url}" target="_blank" rel="noopener" onclick="event.stopPropagation();" style="display:inline-flex;align-items:center;gap:5px;margin-top:4px;color:#2563eb;font-size:12.5px;font-weight:600;text-decoration:none;"><i class="fa-solid fa-up-right-from-square"></i> View on map</a>`;
}
    ...
    modal.innerHTML = `
        <div class="modal-content" style="background: #1e293b; border: 1px solid rgba(255,255,255,0.1); border-radius: 24px; width: 90%; max-width: 760px; overflow: hidden; box-shadow: 0 25px 70px rgba(0,0,0,0.55); position: relative; display: flex; flex-direction: column;">
            <div class="modal-header" style="padding: 24px 32px; display: flex; align-items: center; gap: 18px; border-bottom: 1px solid rgba(255,255,255,0.08); background: #0b2c66; color: white; position: relative;">
                <div id="modalIcon" class="modal-icon-large" style="width: 52px; height: 52px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 24px; background: rgba(255,255,255,0.1); flex-shrink: 0;"></div>
                ...
                <button onclick="window.closeNotificationModal()" style="all: unset; position: absolute; top: 16px; right: 20px; font-size: 32px; color: #94a3b8; cursor: pointer; width: 44px; height: 44px; display: flex; align-items: center; justify-content: center; border-radius: 50%; transition: 0.2s;"
...
function openNotificationModal(report) {
    ...
    let location_lat = report[13];
    let location_lng = report[14];
    ...
    let content = `
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; text-align: left; background: rgba(255,255,255,0.02); padding: 24px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.05); margin-bottom: 8px;">
            ...
            <div style="grid-column: span 2;">
                ...
                ${buildLocationMapHTML(location_lat, location_lng, location)}
            </div>
        </div>
        ${feedbackHTML}
    `;
...
function filterAdminIncidents() {
    ...
    children.forEach(child => {
        ...
        const matchesBreakdown = (activeIncidentTypeFilter === "all" || type.toLowerCase().trim() === activeIncidentTypeFilter.toLowerCase().trim());
        ...
        if (!incidentIdPairs[incId]) {
            incidentIdPairs[incId] = { show: (matchesTime && matchesSearch && matchesBreakdown), elements: [] };
        }
    ...
    if (infoGrid) {
        infoGrid.innerHTML = `
            ...
            <div class="info-item" style="grid-column:1 / -1;">
                ...
                <p>${location}</p>
                ${buildLocationMapHTML(location_lat, location_lng, location, "light")}
            </div>
...
function refreshReports(){
    ...
    .then(reports => {
        ...
        reports.forEach(report => {
            ...
            else if (type === "Fire Alarms and Explosive-Related Incidents") countFire++;
            else if (type === "Damage to Property") countDamage++;
            else if (type === "Theft / Robbery") countTheft++;
            else if (type === "Physical Injuries") countInjury++;
        });
        ...
        const countDamageEl = document.getElementById("countDamage");
        const countTheftEl = document.getElementById("countTheft");
        const countInjuryEl = document.getElementById("countInjury");
        ...
        if (countFireEl) countFireEl.textContent = countFire;
        if (countDamageEl) countDamageEl.textContent = countDamage;
        if (countTheftEl) countTheftEl.textContent = countTheft;
        if (countInjuryEl) countInjuryEl.textContent = countInjury;
        ...
        reports.forEach(report => {
            ...
            card.innerHTML = `
                <div class="report-icon ${status}" style="background: ${iconBg}; border-radius: 18px; display: flex; align-items: center; justify-content: center; font-size: 28px; flex-shrink: 0; box-shadow: 0 8px 20px rgba(0,0,0,0.3); ${iconBorder}">
                ...
                <div class="report-meta" style="margin-right: 40px; margin-left: auto; text-align: right;">
                    ...
                    <div class="time-ago" style="margin-top: 8px; font-size: 14px; color: ${isUnread ? '#cbd5e1' : 'white'}; font-weight: 500;">${getTimeDifference(parseDateSafely(report), new Date())} ago</div>
                </div>
            `;
            ...
            if (incidentGrid) {
                ...
                <div class="detail-card location">
                    ...
                    <div class="content">
                        ...
                        <p>${location}</p>
                        ${buildLocationMapLinkOnly(report[13], report[14], location)}
                    </div>
                </div>
                `;
            ...
            displayReports.forEach(report => {
                ...
                incidentRow.innerHTML = `
                    ...
                    <div class="incident-details">
                        ...
                        ${getTimeDifference(parseDateSafely(report), new Date())} ago
                    </div>
                `;
...
function showErrorModal(title, message, fieldsToHighlight = []) {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="background: #ffffff; border: 1.5px solid #fecaca; border-radius: 20px; width: 100%; max-width: 480px; overflow: hidden; box-shadow: 0 20px 50px rgba(220, 38, 38, 0.15); display: flex; flex-direction: column; font-family: 'Inter', sans-serif;">
            <div class="modal-header" style="padding: 24px; display: flex; align-items: center; gap: 14px; background: #fee2e2; color: #991b1b; border-bottom: 1px solid #fca5a5;">
            ...
            <div class="modal-body" style="padding: 24px; color: #cbd5e1; display: flex; flex-direction: column; overflow-y: auto; max-height: 65vh; border-bottom: 1px solid rgba(255,255,255,0.08);">
                <p style="color: #475569; font-size: 15px; margin: 0; line-height: 1.6;">${message}</p>
            </div>
            <div class="modal-footer" style="padding: 20px 32px; display: flex; justify-content: flex-end; align-items: center; background: #1e293b;">
...
function createNewAccount() {
    ...
    if (isNaN(ageNum) || ageNum < 18 || ageNum > 80) {
        showErrorModal(
            "Invalid Age",
            "Age must be between <strong>18</strong> and <strong>80</strong>.",
            ["newAge"]
        );
        return;
    }
    ...
    if (!/^[0-9]{4}$/.test(badge)) {
        showErrorModal(
            ...
            "Badge Number must be exactly <strong>4 digits</strong> and contain numbers only.",
            ["newBadge"]
        );
        return;
    }
    ...
    if (!/^09[0-9]{9}$/.test(phone)) {
        showErrorModal(
            ...
            "Contact Number must start with <strong>09</strong> and contain exactly <strong>11 digits</strong> (e.g. 09151234567).",
            ["newPhone"]
        );
        return;
    }
    ...
    if (!/^[^\s@]+@gmail\.com$/i.test(email)) {
        showErrorModal(
            ...
            "Email Address must be a valid <strong>@gmail.com</strong> address (e.g. juan.delacruz@gmail.com).",
            ["newEmail"]
        );
        return;
    }
...
function refreshProfileData() {
    ...
    .then(user => {
        ...
        if (personalBirthDate) personalBirthDate.textContent = formatReadableDate(birthday);
        ...
        if (employmentStartDate) employmentStartDate.textContent = formatReadableDate(start_date);
...
function loadSecurityData() {
    ...
    .then(data => {
        ...
        const toggleTFA = document.getElementById("toggleTFA");
        if (toggleTFA) toggleTFA.checked = (data.tfa_enabled === "yes");

        const toggleNotif = document.getElementById("toggleNotif");
        if (toggleNotif) toggleNotif.checked = (data.login_notifications === "yes");

        const toggleActivity = document.getElementById("toggleActivity");
        if (toggleActivity) toggleActivity.checked = (data.activity_logs_enabled === "yes");

        const statTotalLogins = document.getElementById("statTotalLogins");
        if (statTotalLogins) statTotalLogins.textContent = `${data.successful_logins_count || 0} times`;

        const accountStatusBadge = document.getElementById("badgeAccountStatus");
        if (accountStatusBadge) {
            accountStatusBadge.textContent = data.account_status || "Active";
        ...
        if (containerRecentSessions && data.history) {
            ...
            data.history.forEach(session => {
                const device = session[0] || "Unknown Device";
                const loc = session[1] || "Unknown Location";
                const timestamp = session[2] || "Unknown Time";
                const isCurrent = session[3] === "yes";

                const isMobile = device.toLowerCase().includes("phone") ||
                                 device.toLowerCase().includes("android") ||
                                 device.toLowerCase().includes("mobile");

                const sessionHTML = `
                    <div style="background:#f8fafc;padding:16px;border-radius:12px;display:flex;align-items:center;justify-content:space-between;box-shadow:0 4px 15px rgba(0,0,0,0.02);border:1px solid #e2e8f0;">
                        <div style="display:flex;align-items:center;gap:16px;">
                            <i class="fa-solid ${isMobile ? "fa-mobile-screen" : "fa-laptop"}" style="font-size:24px;color:#0b2c66;"></i>
                            <div>
                                <strong style="font-size:15px;color:#0f172a;display:block;">${device}</strong>
                                <span style="color:#64748b;font-size:13px;">${loc} • ${timestamp}</span>
                            </div>
                        </div>
                        ${isCurrent ? '<span style="background:#ecfdf5;color:#166534;padding:4px 12px;border-radius:30px;font-weight:700;font-size:12px;text-transform:uppercase;">Active</span>' : ''}
                    </div>
                `;
...
function updateSecurityLogsUI(logs) {
    ...
    if (container && logs) {
        ...
        logs.forEach(log => {
            const eventType = log[0] || "Event";
            const details = log[1] || "No Details";
            const timestamp = log[2] || "No Timestamp";
...
function loadCertifications() {
    ...
    .then(certs => {
        ...
        certs.forEach(cert => {
            ...
            let issuedStr = formatReadableDate(issued);
            let expiryStr = expiry !== "N/A" ? formatReadableDate(expiry) : "N/A";
    ...
    .then(data => {
        ...
        data.forEach(req => {
            ...
            if (req.change_type === "Employment") {
                ...
                `;
            } else if (req.change_type === "Promotion") {
                detailsHTML = `
                    <strong>New Job Title:</strong> ${parsedData.job_title || "N/A"}<br>
                    <strong>New Department / Division:</strong> ${parsedData.department || "N/A"}<br>
                    <strong>Effective Start Date:</strong> ${formatReadableDate(parsedData.start_date) || "N/A"}
                `;
            } else {
                detailsHTML = `
            ...
            const itemHTML = `
                <div class="pending-item" style="padding: 24px; border-bottom: 1px solid #e2e8f0; display: flex; flex-direction: column; gap: 16px;">
                    ...
                    <div style="display: flex; gap: 12px; justify-content: flex-end;">
                        <button type="button" onclick="reviewProfileChange(${req.id}, 'reject')" style="background: #fee2e2; color: #dc2626; border: none; padding: 10px 20px; border-radius: 8px; cursor: pointer; font-weight: 600; display: flex; align-items: center; gap: 8px;">
                            <i class="fa-solid fa-circle-xmark"></i> Reject
                        </button>
                        <button type="button" onclick="reviewProfileChange(${req.id}, 'approve')" style="background: #10b981; color: white; padding: 12px 28px; border: none; border-radius: 8px; font-weight: 600; cursor: pointer;">
...
function injectUserManualButton() {
    const badgeContainer = document.querySelector(".admin-badge-container");
    if (badgeContainer && !document.getElementById("btnUserManual")) {
        const btn = document.createElement("button");
        btn.type = "button";
        btn.id = "btnUserManual";
        btn.style.cssText = "background: #0b2c66; color: white; padding: 10px 22px; border-radius: 40px; font-weight: 700; font-size: 15px; border: none; cursor: pointer; box-shadow: 0 4px 12px rgba(11, 44, 102, 0.15); display: inline-flex; align-items: center; gap: 8px; margin: 0; transition: background 0.2s;";
        btn.innerHTML = '<i class="fa-solid fa-book-open"></i> User Manual';
        btn.onmouseover = () => btn.style.background = "#1e5799";
        btn.onmouseout = () => btn.style.background = "#0b2c66";
        btn.onclick = openUserManual;
        badgeContainer.insertBefore(btn, badgeContainer.firstChild);
    }
}
...
function openUserManual() {
    currentManualPage = 1;
    let modal = document.getElementById("userManualModal");
    if (!modal) {
        modal = document.createElement("div");
        modal.id = "userManualModal";
        modal.className = "modal-overlay";
        modal.style.cssText = `
            display: none;
            position: fixed;
...
function renderManualPage(page) {
    ...
    if (page === 1) {
        titleText = "I. Introduction & Admin Operations";
        bodyContent = `
            <p style="color: #64748b; font-size: 14px; margin-bottom: 20px;">Welcome, Administrator. This guide walks you through system management and auditing workflows. Here is your standard operational pipeline:</p>

            <!-- Responsive Flowchart -->
            <div style="display: flex; align-items: center; justify-content: space-between; margin: 16px 0 24px; background: #f8fafc; padding: 16px 12px; border-radius: 14px; border: 1.5px solid #cbd5e1; overflow-x: auto; gap: 8px; box-sizing: border-box; width: 100%;">
                <div style="display: flex; flex-direction: column; align-items: center; text-align: center; flex: 1; min-width: 75px; flex-shrink: 0; padding: 4px 0;">
                    <div style="width: 38px; height: 38px; background: #0b2c66; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 6px; font-size: 14px; box-shadow: 0 4px 10px rgba(11,44,102,0.15); flex-shrink: 0;"><i class="fa-solid fa-chart-line"></i></div>
                    <span style="font-size: 10px; font-weight: 700; color: #0f172a; white-space: nowrap;">1. Telemetry</span>
                </div>
                <i class="fa-solid fa-chevron-right" style="color: #94a3b8; font-size: 11px; flex-shrink: 0; align-self: center; margin-bottom: 14px;"></i>
                <div style="display: flex; flex-direction: column; align-items: center; text-align: center; flex: 1; min-width: 75px; flex-shrink: 0; padding: 4px 0;">
                    <div style="width: 38px; height: 38px; background: #0b2c66; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 6px; font-size: 14px; box-shadow: 0 4px 10px rgba(11,44,102,0.15); flex-shrink: 0;"><i class="fa-solid fa-magnifying-glass"></i></div>
                    <span style="font-size: 10px; font-weight: 700; color: #0f172a; white-space: nowrap;">2. Feeds</span>
                </div>
                <i class="fa-solid fa-chevron-right" style="color: #94a3b8; font-size: 11px; flex-shrink: 0; align-self: center; margin-bottom: 14px;"></i>
                <div style="display: flex; flex-direction: column; align-items: center; text-align: center; flex: 1; min-width: 75px; flex-shrink: 0; padding: 4px 0;">
                    <div style="width: 38px; height: 38px; background: #0b2c66; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 6px; font-size: 14px; box-shadow: 0 4px 10px rgba(11,44,102,0.15); flex-shrink: 0;"><i class="fa-solid fa-file-signature"></i></div>
                    <span style="font-size: 10px; font-weight: 700; color: #0f172a; white-space: nowrap;">3. Reports</span>
                </div>
                <i class="fa-solid fa-chevron-right" style="color: #94a3b8; font-size: 11px; flex-shrink: 0; align-self: center; margin-bottom: 14px;"></i>
                <div style="display: flex; flex-direction: column; align-items: center; text-align: center; flex: 1; min-width: 75px; flex-shrink: 0; padding: 4px 0;">
                    <div style="width: 38px; height: 38px; background: #0b2c66; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 6px; font-size: 14px; box-shadow: 0 4px 10px rgba(11,44,102,0.15); flex-shrink: 0;"><i class="fa-solid fa-clipboard-check"></i></div>
                    <span style="font-size: 10px; font-weight: 700; color: #0f172a; white-space: nowrap;">4. Profiles</span>
                </div>
                <i class="fa-solid fa-chevron-right" style="color: #94a3b8; font-size: 11px; flex-shrink: 0; align-self: center; margin-bottom: 14px;"></i>
                <div style="display: flex; flex-direction: column; align-items: center; text-align: center; flex: 1; min-width: 75px; flex-shrink: 0; padding: 4px 0;">
                    <div style="width: 38px; height: 38px; background: #0b2c66; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 6px; font-size: 14px; box-shadow: 0 4px 10px rgba(11,44,102,0.15); flex-shrink: 0;"><i class="fa-solid fa-users-cog"></i></div>
                    <span style="font-size: 10px; font-weight: 700; color: #0f172a; white-space: nowrap;">5. Users</span>
                </div>
            </div>

            <h4 style="color: #0b2c66; font-size: 16px; font-weight: 800; margin-bottom: 14px; text-align: left; display: flex; align-items: center; gap: 8px;">
                <i class="fa-solid fa-bars-staggered" style="color: #0b2c66;"></i> Command Center Sidebars
            </h4>
            <div style="display: flex; flex-direction: column; gap: 12px; text-align: left;">
                <div style="display: flex; align-items: flex-start; gap: 12px;">
                    <i class="fa-solid fa-gauge-high" style="color: #0b2c66; font-size: 16px; margin-top: 3px; width: 20px; text-align: center; flex-shrink: 0;"></i>
                    <div><strong>Dashboard</strong>: Global live telemetry tracking, pending logs lists, and checklist configurations.</div>
                </div>
                <div style="display: flex; align-items: flex-start; gap: 12px;">
                    <i class="fa-solid fa-triangle-exclamation" style="color: #0b2c66; font-size: 16px; margin-top: 3px; width: 20px; text-align: center; flex-shrink: 0;"></i>
                    <div><strong>Incidents</strong>: Sector traffic trend breakdown statistics and responsive detail video grids.</div>
                </div>
                <div style="display: flex; align-items: flex-start; gap: 12px;">
                    <i class="fa-solid fa-file-lines" style="color: #0b2c66; font-size: 16px; margin-top: 3px; width: 20px; text-align: center; flex-shrink: 0;"></i>
                    <div><strong>Reports</strong>: Case audit controls, decision processing (Approve, Deny, Revisions), and Excel exports.</div>
                </div>
                <div style="display: flex; align-items: flex-start; gap: 12px;">
                    <i class="fa-solid fa-user-shield" style="color: #0b2c66; font-size: 16px; margin-top: 3px; width: 20px; text-align: center; flex-shrink: 0;"></i>
                    <div><strong>Profile</strong>: Personal identity sheets, certifications, proposed update reviewers, and security logs.</div>
                </div>
                <div style="display: flex; align-items: flex-start; gap: 12px;">
                    <i class="fa-solid fa-bell" style="color: #0b2c66; font-size: 16px; margin-top: 3px; width: 20px; text-align: center; flex-shrink: 0;"></i>
                    <div><strong>Notifications</strong>: Unread alerts indicator and click-to-review prompts with print summary actions.</div>
                </div>
                <div style="display: flex; align-items: flex-start; gap: 12px;">
                    <i class="fa-solid fa-users-cog" style="color: #0b2c66; font-size: 16px; margin-top: 3px; width: 20px; text-align: center; flex-shrink: 0;"></i>
                    <div><strong>User Management</strong>: Complex registration controls, soft suspensions, and total database purges.</div>
                </div>
            </div>
        `;
    } else if (page === 2) {
        titleText = "II. Dashboard Telemetry & Checkpoints";
        bodyContent = `
            <p style="color: #64748b; font-size: 14px; margin-bottom: 20px;">Govern daily checkpoint goals and study real-time precinct telemetry indicators:</p>
            <div style="display: flex; flex-direction: column; gap: 14px; text-align: left;">
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-list-check" style="color: #0b2c66; font-size: 16px;"></i> On-Duty Checklist Tasks
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Manage daily checkpoints. Click "Add New Task", input parameters with priority tags, and save. Click "X" to clear items.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-chart-line" style="color: #0b2c66; font-size: 16px;"></i> Dynamic Stat Aggregates
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Review computed metrics of Recent Incidents, Resolved Cases verified today, total Personnel on Duty, and Average response delay.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-video" style="color: #0b2c66; font-size: 16px;"></i> On-Scene Compiled Streams
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">The bottom feed lists recent incident filings. Click any card to launch detailed evaluations immediately.</p>
                </div>
            </div>
        `;
    } else if (page === 3) {
        titleText = "III. STUDY Incident Trends & Grids";
        bodyContent = `
            <p style="color: #64748b; font-size: 14px; margin-bottom: 20px;">Explore, filter, and study recorded traffic compiled across sector precincts:</p>
            <div style="display: flex; flex-direction: column; gap: 14px; text-align: left;">
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-table-cells-large" style="color: #0b2c66; font-size: 16px;"></i> Dynamic Breakdown Filters
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Select any breakdown card (DUI, Hit & Run, Pileup) to filter the grid instantly. Click again to reset.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-magnifying-glass" style="color: #0b2c66; font-size: 16px;"></i> Study Controls
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Search locations, officers, or case IDs. Limit listings by time scopes (24 hours, week, or month) or sort chronologically.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-list-ul" style="color: #0b2c66; font-size: 16px;"></i> Video Details Split Grid
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Click "Toggle Details Grid" to shift layouts between full video feeds or metadata split view.</p>
                </div>
            </div>
        `;
    } else if (page === 4) {
        titleText = "IV. Report Verification & Excel Export";
        bodyContent = `
            <p style="color: #64748b; font-size: 14px; margin-bottom: 20px;">Process pending files in the verification queue with complete administrative oversight:</p>
            <div style="display: flex; flex-direction: column; gap: 14px; text-align: left;">
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-gavel" style="color: #0b2c66; font-size: 16px;"></i> Evaluation Choices
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Examine video evidence and parameters. Process evaluations by clicking <strong>Approve</strong>, <strong>Deny</strong>, or <strong>Request Changes</strong> (notes required).</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-file-excel" style="color: #0b2c66; font-size: 16px;"></i> Spreadsheet Export
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Click "Export All to Excel" inside the Reports view to compile and download chronologically formatted records.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-print" style="color: #0b2c66; font-size: 16px;"></i> PDF Printing Summaries
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Generate formatted summaries for physical filing by launching print actions inside any detailed report modal.</p>
                </div>
            </div>
        `;
    } else if (page === 5) {
        titleText = "V. Profiles, Licenses, & Security Settings";
        bodyContent = `
            <p style="color: #64748b; font-size: 14px; margin-bottom: 20px;">Audit administrative credentials, professional certifications, and security metrics:</p>
            <div style="display: flex; flex-direction: column; gap: 14px; text-align: left;">
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-user-edit" style="color: #0b2c66; font-size: 16px;"></i> Profile Sheets
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Modify contacts, emergency details, or deployment history. Admin changes commit instantly to the database.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-award" style="color: #0b2c66; font-size: 16px;"></i> Certifications & Licenses
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Record academy achievements by logging issuing organizations, dates, and uploading documentation.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-shield-halved" style="color: #0b2c66; font-size: 16px;"></i> Security Controls & Audit Trails
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Toggle Email 2FA OTP codes, manage backup recovery keys, reset password PINs, and audit system-wide security logs.</p>
                </div>
            </div>
        `;
    } else if (page === 6) {
        titleText = "VI. Profile Review & User Registries";
        bodyContent = `
            <p style="color: #64748b; font-size: 14px; margin-bottom: 20px;">Govern officer proposed profile changes and configure platform access permissions:</p>
            <div style="display: flex; flex-direction: column; gap: 14px; text-align: left;">
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-user-pen" style="color: #0b2c66; font-size: 16px;"></i> Profile Change Proposals
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Review proposed adjustments submitted by general officers. Approved promotions automatically transition prior roles into dynamic history items.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-user-plus" style="color: #0b2c66; font-size: 16px;"></i> Credentials Creation
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Create new accounts under User Management. Select Employee or Admin roles and specify robust password credentials adhering to system rules.</p>
                </div>
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #cbd5e1; box-sizing: border-box;">
                    <strong style="color: #0b2c66; font-size: 15px; display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                        <i class="fa-solid fa-user-slash" style="color: #0b2c66; font-size: 16px;"></i> Account Suspension & Purging
                    </strong>
                    <p style="margin: 0; line-height: 1.55; font-size: 13.5px; color: #334155;">Choose <strong>Deactivate</strong> to instantly block login permissions while preserving historical reports, or <strong>Permanently Delete</strong> to purge records completely.</p>
                </div>
            </div>
        `;
    }
    ...
    modal.innerHTML = `
        <div class="modal-content user-manual-content">
            ...
            <div class="modal-header user-manual-header">
                ...
                <button type="button" class="user-manual-close" onclick="closeUserManual()">&times;</button>
            </div>
            ...
            <!-- FOOTER -->
            <div class="modal-footer user-manual-footer">
                <button type="button" class="manual-nav-btn manual-btn-back" onclick="renderManualPage(${page - 1})" ${page === 1 ? 'disabled' : ''}>
                    <i class="fa-solid fa-arrow-left"></i> Back
                </button>

                <div class="user-manual-dots">
                    <span class="dot ${page === 1 ? 'active' : ''}"></span>
                    <span class="dot ${page === 2 ? 'active' : ''}"></span>
                    <span class="dot ${page === 3 ? 'active' : ''}"></span>
                    <span class="dot ${page === 4 ? 'active' : ''}"></span>
                    <span class="dot ${page === 5 ? 'active' : ''}"></span>
                    <span class="dot ${page === 6 ? 'active' : ''}"></span>
                </div>

                ${page < 6 ? `
                    <button type="button" class="manual-nav-btn manual-btn-next" onclick="renderManualPage(${page + 1})">
                        Next <i class="fa-solid fa-arrow-right"></i>
                    </button>
                ` : `
                    <button type="button" class="manual-nav-btn manual-btn-finish" onclick="closeUserManual()">
                        Finish
                    </button>
                `}
            </div>
...
document.addEventListener("DOMContentLoaded", () => {
    // --- ACCITRACK SECURITY INPUT VALIDATION ---

    // 1. Text Fields: Strictly letters and spaces (strips numbers and all special characters)
    const strictlyLettersIds = [
        "editFirst", "editMiddle", "editLast", "editNation", "editEmergName", "editEmergRel",
        "editEmploymentSupervisor", "newName",
        "obFirst", "obMiddle", "obLast", "obPref", "obNation", "obEmergName", "obEmergRel", "obSupervisor"
    ];
    strictlyLettersIds.forEach(id => {
    ...
    // 2. Alphanumeric and Spaces (letters, numbers, spaces - no special characters)
    const alphanumericSpaceIds = [
        "editEmploymentJobTitle", "editEmploymentDepartment", "editEmploymentType", "editEmploymentLocation", "editEmploymentSchedule",
        "obJobTitle", "obDepartment", "obEmpType", "obWorkLocation", "obSchedule"
    ];
    alphanumericSpaceIds.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            el.addEventListener("input", function() {
    ...
    // 3. Email Fields: Alphanumeric and standard characters (@, ., _, -)
    const emailInputIds = ["newEmail", "editContactEmail", "obEmail"];
    ...
    // 4. Phone Fields: Strictly 11 numbers max with fixed layout formatting (XXXX-XXX-XXXX)
    const strictlyNumbersIds = [
        "editContactPhoneWork", "editContactPhoneMobile", "editEmergPhone",
        "obWorkPhone", "obMobilePhone", "obEmergPhone"
    ];
    strictlyNumbersIds.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            el.addEventListener("input", function() {
                // Strip all characters that aren't numeric digits
                let digits = this.value.replace(/[^0-9]/g, "");

                // Restrict input to a maximum of 11 characters
                if (digits.length > 11) {
                    digits = digits.slice(0, 11);
                }

                // Formulate layout formatting dynamically
                let formatted = "";
                if (digits.length > 0) {
                    if (digits.length <= 4) {
                        formatted = digits;
    ...
    // Handle Age Field separately (Strictly digits only, max 2 digits)
    const newAgeEl = document.getElementById("newAge");
    if (newAgeEl) {
        newAgeEl.addEventListener("input", function() {
            this.value = this.value.replace(/[^0-9]/g, "").slice(0, 2);
        });
    }

    // 5. Restrict all password fields to prevent spaces entirely
    const passwordFields = ["newPin", "changePassCurrent", "changePassNew", "changePassConfirm"];
    ...
    const breakdownCards = document.querySelectorAll(".accident-breakdown-row .breakdown-card");
    breakdownCards.forEach(card => {
        card.addEventListener("click", () => {
            const type = card.getAttribute("data-type");
            if (activeIncidentTypeFilter === type) {
                activeIncidentTypeFilter = "all";
            } else {
                activeIncidentTypeFilter = type;
            }
            updateBreakdownCardSelectionVisuals(activeIncidentTypeFilter);
            filterAdminIncidents();
        });
    });
});
...
// --- TRIGGER METHOD FOR EXPORTING EXCEL SPREADSHEETS ---
function exportReportsToExcel() {
    window.location.href = "/export-all-reports-excel";
}
