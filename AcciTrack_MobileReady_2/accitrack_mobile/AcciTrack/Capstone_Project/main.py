...
PI_API_KEY = os.environ.get("PI_API_KEY", "change-me-shared-secret")  # must match the Pi script's UPLOAD_API_KEY
...
def camera():
    ...
    back_url = '/admin' if role == 'admin' else '/client'
...
def pi_crash_report():
    """
    Dedicated endpoint for the Raspberry Pi crash-detection unit to submit
    incident reports directly — no browser session required. Authenticated
    via a shared API key header instead, since the Pi isn't a logged-in user.
    """
    if request.headers.get("X-API-KEY") != PI_API_KEY:
    ...
    try:
        ...
        if file.filename == '' or not allowed_file(file.filename):
            return "Fail: invalid file", 400

        filename = secure_filename(file.filename)
        file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        video_path = "/upload/" + filename

        location = request.form.get("location", "Unspecified - Pi Camera Unit")
        now = datetime.datetime.now()
        datetime_str = now.strftime("%b %d %Y %I:%M:%S %p")
        case_num = f"CRA-{now.year}-{random.randint(1000, 9999)}"

        values = [
            {"report_caseNum": case_num},
            {"report_submitting_officer": "AI Crash Detection System"},
            {"report_submitting_datetime": datetime_str},
            {"report_location": location},
            {"report_type": "Vehicle Collision (AI Detected)"},
            ...
            {"report_reviewing_reason": "none"},
            {"report_is_read": "no"},
            {"report_officer_badge_number": "0"},
            {"report_real_submission_datetime": datetime_str}
        ]
        db.insert_to_table("AcciTrack", "AcciTrack_ReportList", values)

        log_security_event("0", "AI Report Submitted",
                           f"Automated crash detection system submitted report {case_num} at {location}.")
