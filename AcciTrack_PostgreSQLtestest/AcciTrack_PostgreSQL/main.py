    ...
    try:
        ...
        for officer in db.get_table_values("AcciTrack", "AcciTrack_OfficerList"):
            if officer[9] == badge_number or officer[30] == username:
                ...
                try:
                    ...
                    cursor = conn.cursor()
                    cursor.execute("SELECT account_status FROM AcciTrack_SecuritySettings WHERE badge_number = %s",
                                   (officer[9],))
                ...
                if officer_pin == officer[29]:
                    ...
                    if is_admin:
                        try:
                            conn = get_db_connection()
                            cursor = conn.cursor()
                            cursor.execute("SELECT tfa_enabled FROM AcciTrack_SecuritySettings WHERE badge_number = %s",
                                           (officer[9],))
                    ...
                    try:
                        ...
                        cursor.execute("""
                            ...
                            VALUES (%s, 'Successful Login', 'Logged in via Windows PC at Headquarters', %s)
                        """, (officer[9], timestamp))
        ...
        if badge_number:
            try:
                ...
                cursor.execute("""
                    INSERT INTO AcciTrack_SecurityLogs (badge_number, event_type, details, timestamp)
                    VALUES (%s, 'Failed Login Attempt', 'Incorrect credentials entered from IP 203.0.113.45', %s)
...
def send_recovery_email():
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT officer_primary_email FROM AcciTrack_OfficerList WHERE officer_badge_number = %s",
                       (badge,))
...
def verify_email_reset():
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE AcciTrack_OfficerList SET officer_pin = %s WHERE officer_badge_number = %s",
                       (new_pin, badge))
        conn.commit()
        conn.close()
...
def verify_login_2fa():
    try:
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO AcciTrack_SecurityLogs (badge_number, event_type, details, timestamp)
                VALUES (%s, 'Successful Login', 'Logged in via Windows PC using Two-Factor Authentication', %s)
            """, (badge_number, timestamp))
...
def get_tasks():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id, task_title, task_description, task_priority 
            FROM AcciTrack_TaskList 
            WHERE task_officer_badge_number = %s
        """, (badge,))
...
def delete_task():
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM AcciTrack_TaskList WHERE id = %s AND task_officer_badge_number = %s",
                       (task_id, badge))
        conn.commit()
        conn.close()
        return "Success"
...
def mark_report_read():
    try:
        ...
        cursor.execute("SELECT officer_is_admin FROM AcciTrack_OfficerList WHERE officer_badge_number = %s", (badge,))
        ...
        if is_admin:
            cursor.execute("""
                ...
                WHERE report_caseNum = %s
            """, (case_num,))
        else:
            cursor.execute("""
                UPDATE AcciTrack_ReportList 
                SET report_is_read = 'yes'
                WHERE report_caseNum = %s AND report_officer_badge_number = %s
            """, (case_num, badge))
        conn.commit()
        conn.close()
        return "Success"
...
def update_report_status():
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT officer_preferred_name FROM AcciTrack_OfficerList WHERE officer_badge_number = %s",
                       (badge,))
        row = cursor.fetchone()
        if row and row[0]:
        ...
        cursor.execute("""
            UPDATE AcciTrack_ReportList
            SET report_status = %s,
                report_reviewing_officer = %s,
                report_reviewing_datetime = %s,
                report_reviewing_reason = %s,
                report_is_read = 'no'
            WHERE report_caseNum = %s
...
def delete_user():
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT officer_is_admin FROM AcciTrack_OfficerList WHERE officer_badge_number = %s",
                       (admin_badge,))
        row = cursor.fetchone()
        ...
        cursor.execute(
            "SELECT officer_first_name, officer_last_name FROM AcciTrack_OfficerList WHERE officer_badge_number = %s",
            (target_badge,))
        ...
        if action == "deactivate":
            ...
            cursor.execute("SELECT 1 FROM AcciTrack_SecuritySettings WHERE badge_number = %s", (target_badge,))
            if not cursor.fetchone():
                cursor.execute("""
                    ...
                    VALUES (%s, 'no', 'yes', 'yes', '4829-1746,7391-2058,1567-8932,6041-5279,9283-4165', 'Inactive')
                """, (target_badge,))
            else:
                cursor.execute(
                    "UPDATE AcciTrack_SecuritySettings SET account_status = 'Inactive' WHERE badge_number = %s",
                    (target_badge,))
            ...
            cursor.execute("DELETE FROM AcciTrack_OfficerList WHERE officer_badge_number = %s", (target_badge,))
            cursor.execute("DELETE FROM AcciTrack_SecuritySettings WHERE badge_number = %s", (target_badge,))
            cursor.execute("DELETE FROM AcciTrack_AccessHistory WHERE badge_number = %s", (target_badge,))
...
def delete_certification():
    try:
        ...
        cursor.execute("SELECT cert_name, file_path FROM AcciTrack_Certifications WHERE id = %s AND badge_number = %s",
                       (cert_id, badge))
        row = cursor.fetchone()
        ...
        cursor.execute("DELETE FROM AcciTrack_Certifications WHERE id = %s AND badge_number = %s", (cert_id, badge))
        conn.commit()
        conn.close()
        ...
        if is_admin:
            if change_type == "Employment":
                cursor.execute("""
                    ...
                    SET officer_employment_job_title = %s,
                        officer_employment_department = %s,
                        officer_employment_type = %s,
                        officer_employment_reporting_officer = %s,
                        officer_employment_work_location = %s
                    WHERE officer_badge_number = %s
                ...
                cursor.execute("""
                    UPDATE AcciTrack_OfficerList
                    SET officer_employment_history = %s
                    WHERE officer_badge_number = %s
                """, (requested_data.get("history_data"), badge))
            else:
                cursor.execute("""
                    UPDATE AcciTrack_OfficerList
                    SET officer_primary_email = %s,
                        officer_secondary_email = %s,
                        officer_work_phone = %s,
                        officer_mobile_phone = %s,
                        officer_primary_contact_name = %s,
                        officer_primary_contact_phone_number = %s,
                        officer_primary_contact_relationship = %s,
                        officer_secondary_contact_name = %s,
                        officer_secondary_contact_phone_number = %s,
                        officer_secondary_contact_relationship = %s
                    WHERE officer_badge_number = %s
...
def update_personal_details():
    try:
        ...
        url = f"UPDATE AcciTrack_OfficerList SET officer_first_name = %s, officer_middle_name = %s, officer_last_name = %s, officer_birthday = %s, officer_gender = %s, officer_nationality = %s WHERE officer_badge_number = %s"
        conn = get_db_connection()
        cursor = conn.cursor()
...
def update_contact_details():
    try:
        ...
        url = f"UPDATE AcciTrack_OfficerList SET officer_primary_email = %s, officer_secondary_email = %s, officer_work_phone = %s, officer_mobile_phone = %s WHERE officer_badge_number = %s"
        conn = get_db_connection()
        cursor = conn.cursor()
...
def update_emergency_details():
    try:
        ...
        url = """UPDATE AcciTrack_OfficerList 
                 SET officer_primary_contact_name = %s, 
                     officer_primary_contact_relationship = %s, 
                     officer_primary_contact_phone_number = %s, 
                     officer_secondary_contact_name = %s, 
                     officer_secondary_contact_relationship = %s, 
    ...
    try:
        ...
        url = """UPDATE AcciTrack_OfficerList 
                 SET officer_employment_job_title = %s, 
                     officer_employment_department = %s, 
                     officer_employment_type = %s, 
                     officer_employment_start_date = %s, 
                     officer_employment_reporting_officer = %s, 
                     officer_employment_work_location = %s,
                     officer_employment_history = %s
                 WHERE officer_badge_number = %s"""
        conn = get_db_connection()
        cursor = conn.cursor()
...
def upload_document():
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT officer_document_list FROM AcciTrack_OfficerList WHERE officer_badge_number = %s",
                       (badge,))
        row = cursor.fetchone()
        ...
        cursor.execute("UPDATE AcciTrack_OfficerList SET officer_document_list = %s WHERE officer_badge_number = %s",
                       (updated_docs, badge))
        conn.commit()
        conn.close()
...
def change_pin():
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT officer_pin FROM AcciTrack_OfficerList WHERE officer_badge_number = %s", (badge,))
        row = cursor.fetchone()
...
def reset_pin_backup_code():
    try:
        ...
        cursor.execute("SELECT officer_badge_number FROM AcciTrack_OfficerList WHERE officer_badge_number = %s",
                       (badge,))
        user = cursor.fetchone()
        if not user:
            conn.close()
        ...
        cursor.execute("SELECT backup_codes FROM AcciTrack_SecuritySettings WHERE badge_number = %s", (badge,))
        row = cursor.fetchone()
        ...
        cursor.execute("UPDATE AcciTrack_OfficerList SET officer_pin = %s WHERE officer_badge_number = %s",
                       (new_pin, badge))
        cursor.execute("UPDATE AcciTrack_SecuritySettings SET backup_codes = %s WHERE badge_number = %s",
...
def request_profile_picture():
    try:
        ...
        cursor.execute(
            "SELECT requested_data FROM AcciTrack_ProfileChanges WHERE badge_number = %s AND change_type = 'ProfilePicture' AND status = 'pending'",
            (badge,))
        ...
        cursor.execute(
            "DELETE FROM AcciTrack_ProfileChanges WHERE badge_number = %s AND change_type = 'ProfilePicture' AND status = 'pending'",
            (badge,))
        ...
        cursor.execute("""
            INSERT INTO AcciTrack_ProfileChanges (badge_number, change_type, requested_data, timestamp, status)
            VALUES (%s, 'ProfilePicture', %s, %s, 'pending')
...
def check_pending_avatar():
    try:
        ...
        cursor.execute("""
            ...
            WHERE badge_number = %s AND change_type = 'ProfilePicture' AND status = 'pending'
        """, (badge,))
        row = cursor.fetchone()
        conn.close()
...
def get_security_settings():
    try:
        ...
        cursor.execute("SELECT * FROM AcciTrack_SecuritySettings WHERE badge_number = %s", (badge,))
        ...
        cursor.execute(
            "SELECT device, location, timestamp, is_current FROM AcciTrack_AccessHistory WHERE badge_number = %s ORDER BY id DESC",
            (badge,))
        ...
        if is_admin:
            ...
            cursor.execute(
                "SELECT event_type, details, timestamp FROM AcciTrack_SecurityLogs WHERE badge_number = %s ORDER BY id DESC",
                (badge,))
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM AcciTrack_SecurityLogs WHERE badge_number = %s AND event_type = 'Successful Login'",
            (badge,))
...
def update_security_toggle():
    try:
        ...
        if toggle_type == "tfa":
            cursor.execute("UPDATE AcciTrack_SecuritySettings SET tfa_enabled = %s WHERE badge_number = %s",
                           (value, badge))
            log_security_event(badge, "2FA Toggle", f"Two-Factor Authentication toggled to {value.upper()}")
        elif toggle_type == "notif":
            cursor.execute("UPDATE AcciTrack_SecuritySettings SET login_notifications = %s WHERE badge_number = %s",
                           (value, badge))
            log_security_event(badge, "Notification Toggle", f"Login Notifications toggled to {value.upper()}")
        elif toggle_type == "activity":
            cursor.execute("UPDATE AcciTrack_SecuritySettings SET activity_logs_enabled = %s WHERE badge_number = %s",
                           (value, badge))
...
def review_profile_change():
    try:
        ...
        cursor.execute("SELECT badge_number, change_type, requested_data FROM AcciTrack_ProfileChanges WHERE id = %s",
                       (request_id,))
        ...
        if action == "approve":
            if change_type == "Employment":
                ...
                cursor.execute("""
                    UPDATE AcciTrack_OfficerList
                    SET officer_first_name = %s,
                        officer_middle_name = %s,
                        officer_last_name = %s,
                        officer_birthday = %s,
                        officer_gender = %s,
                        officer_nationality = %s
                    WHERE officer_badge_number = %s
                """, (
            ...
            cursor.execute("UPDATE AcciTrack_ProfileChanges SET status = 'approved' WHERE id = %s", (request_id,))
            conn.commit()
            conn.close()
                ...
                if pending_path:
                    ...
                    if os.path.exists(pending_file_path):
                        try:
                            ...
                            pass
            cursor.execute("UPDATE AcciTrack_ProfileChanges SET status = 'rejected' WHERE id = %s", (request_id,))
...
def check_onboarding():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT account_status FROM AcciTrack_SecuritySettings WHERE badge_number = %s", (badge,))
        row = cursor.fetchone()
        conn.close()
...
def complete_onboarding():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE AcciTrack_SecuritySettings SET account_status = 'Active' WHERE badge_number = %s",
                       (badge,))
        conn.commit()
        conn.close()
...
def seed_custom_users():
    ...
    try:
        ...
        if "2" not in existing_badges and "oogabooga" not in existing_usernames:
            ...
            # Self-healing migration to update any existing Mejiro email address
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("""
                UPDATE AcciTrack_OfficerList 
...
if "__main__" == __name__:
    # CREATE TABLE IF NOT EXISTS is idempotent in Postgres, so just always
    # ensure the schema exists rather than checking for a sqlite file.
