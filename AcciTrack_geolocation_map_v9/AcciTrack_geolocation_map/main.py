...
def resolve_profile_picture_path(badge):
    """
    Return the URL for a badge's current avatar, or the default icon if
    none has been uploaded yet. Used both by the /get-profile-picture
    endpoint (for JS-driven updates) and by page routes (so the <img> tag
    is already correct on first render, with no fetch-then-swap flash).
    """
        ...
        for officer in db.get_table_values("AcciTrack", "AcciTrack_OfficerList"):
            if officer[9] == badge_number or officer[30] == username:
                ...
                if officer[9] != badge_number:
                    increment_failed_attempt(badge_number)
