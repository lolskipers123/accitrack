...
def enforce_https():
    """
    Browsers (iOS Safari in particular) refuse to expose the Geolocation API
    on plain HTTP, and fail silently/immediately with PERMISSION_DENIED
    rather than showing a real error. Render terminates TLS at its edge and
    forwards the original scheme via X-Forwarded-Proto, so we check that
    header (not request.is_secure, which would otherwise always look
    "insecure" behind the proxy) and redirect to HTTPS if needed.
    Only applied when actually running on Render, so local http development
    is unaffected.
    """
