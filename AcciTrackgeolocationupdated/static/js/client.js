...
// iOS Safari has a known quirk where the very first getCurrentPosition()
// call after granting permission can immediately fail with
// POSITION_UNAVAILABLE or TIMEOUT even though the device clearly has a
// location fix (it needs a moment to actually acquire GPS/Wi-Fi
// positioning). A quick, lower-accuracy retry almost always succeeds right
// after. This wraps that retry so callers just get one success/failure.
function requestGeolocationWithRetry(onSuccess, onFailure, isRetry = false) {
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        function (error) {
            ...
            if (canRetry) {
                requestGeolocationWithRetry(onSuccess, onFailure, true);
            } else {
                onFailure(error);
            }
        },
